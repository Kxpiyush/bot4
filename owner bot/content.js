document['addEventListener']('DOMContentLoaded', async function () {
    ...
    document['getElementById']('clear-all')['addEventListener']('click', r), document['getElementById']('city-input')['addEventListener']('keyup', function (w) {
        ...
        if (w['action'] === 'playSound') {
            console['log']('Attempting\x20to\x20play\x20sound\x20using\x20Web\x20Audio\x20API...');
            const z = new (window['AudioContext'] || window['webkitAudioContext'])();
            fetch(chrome['runtime']['getURL']('alert.wav'))['then'](A => A['arrayBuffer']())['then'](A => z['decodeAudioData'](A))['then'](A => {
                const B = z['createBufferSource']();
                B['buffer'] = A, B['connect'](z['destination']), B['start'](0x0), console['log']('Sound\x20played\x20successfully\x20via\x20Web\x20Audio\x20API'), setTimeout(() => {
                    const C = z['createBufferSource']();
                    C['buffer'] = A, C['connect'](z['destination']), C['start'](0x0);
                }, 0x3e8), setTimeout(() => {
                    const C = z['createBufferSource']();
                    C['buffer'] = A, C['connect'](z['destination']), C['start'](0x0);
                }, 0x7d0);
            })['catch'](A => {
                console['error']('Failed\x20to\x20play\x20sound\x20via\x20Web\x20Audio\x20API:', A);
                const B = new Audio(chrome['runtime']['getURL']('alert.wav'));
                B['play']()['catch'](C => console['error']('Also\x20failed\x20with\x20HTML5\x20Audio:', C)), chrome['notifications']['create']({
                    'type': 'basic',
